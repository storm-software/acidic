...
# Acidic Template Engine Crate

A core crate containing the Acidic Template Engine. It is used by various other Acidic packages to generate code based on the user's provided schema.
...
```toml
[dependencies]
acidic-template-engine = "0.0.1"
```

Or, via the command line:

```bash
cargo add acidic-template-engine
```
...
Run `nx build acidic-template-engine` to build the library.

### Running unit tests

Run `nx test acidic-template-engine` to execute the unit tests via [Jest](https://jestjs.io).

### Linting

Run `nx lint acidic-template-engine` to run [ESLint](https://eslint.org/) on the package.

...
A core crate containing the Acidic schema definitions and validation logic. It is used by various other Acidic packages to run their processes.
...
Run `nx build acidic-schema` to build the library.

### Running unit tests

Run `nx test acidic-schema` to execute the unit tests via [Jest](https://jestjs.io).

### Linting

Run `nx lint acidic-schema` to run [ESLint](https://eslint.org/) on the package.

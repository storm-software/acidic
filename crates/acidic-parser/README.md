...
A core crate containing the Acidic parser definitions and validation logic. It is used by various other Acidic packages to run their processes.

## Installing

This crate can be installed using [Cargo](https://doc.rust-lang.org/stable/cargo/) by adding it to your `Cargo.toml` file:
...
This crate is built using [Nx](https://nx.dev). As a result, many of the usual commands are available to assist in development.

### Building

Run `nx build acidic-parser` to build the library.

### Running unit tests

Run `nx test acidic-parser` to execute the unit tests via [Jest](https://jestjs.io).

### Linting

Run `nx lint acidic-parser` to run [ESLint](https://eslint.org/) on the package.

...
# Acidic Diagnostics Crate

A core crate containing the Acidic diagnostics definitions and validation logic. It is used by various other Acidic packages to run their processes.
...
```toml
[dependencies]
acidic-diagnostics = "0.0.1"
```

Or, via the command line:

```bash
cargo add acidic-diagnostics
```
...
Run `nx build acidic-diagnostics` to build the library.

### Running unit tests

Run `nx test acidic-diagnostics` to execute the unit tests via [Jest](https://jestjs.io).

### Linting

Run `nx lint acidic-diagnostics` to run [ESLint](https://eslint.org/) on the package.

...
# Acidic Daemon Process

A package containing the Acidic Engine's daemon process code to allow automatically updated code/artifacts.
...
Run `nx build daemon` to build the library.

### Running unit tests

Run `nx test daemon` to execute the unit tests via [Jest](https://jestjs.io).

### Linting

Run `nx lint daemon` to run [ESLint](https://eslint.org/) on the package.

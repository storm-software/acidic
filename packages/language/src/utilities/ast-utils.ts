import type { AstNode, Mutable } from "langium";
import {
  type AcidicEvent,
  type AcidicModel,
  type AcidicMutation,
...
export function extractAcidicModelsWithAllowRules(schema: AcidicSchema): AcidicModel[] {
  return schema.declarations.filter(
    (d) => isAcidicModel(d) && d.attributes.some((attr) => attr.decl.ref?.name === "@@allow")
  ) as AcidicModel[];
}

export function mergeBaseSchema(schema: AcidicSchema) {
  for (const decl of schema.declarations.filter(
    (x) => x.$type === "AcidicObject" || x.$type === "AcidicModel"
  )) {
...
export function getIdFields(model: AcidicModel | AcidicObject) {
  ...
  const modelIdAttr = model.attributes.find((attr) => attr.decl?.ref?.name === "@@id");
  if (modelIdAttr && Array.isArray(modelIdAttr.args) && modelIdAttr.args.length > 0) {
...
export function isAuthInvocation(node: AstNode) {
  return (
    isInvocationExpr(node) && node.function.ref?.name === "auth" && isFromStdlib(node.function.ref)
  );
}

export function getAcidicObjectReference(expr: Expression): AcidicObject | undefined {
  if (isReferenceExpr(expr) && isAcidicObject(expr.target.ref)) {
    return expr.target.ref;
  }
  if (isMemberAccessExpr(expr) && isAcidicObject(expr.member.ref)) {
    return expr.member.ref;
  }
  return undefined;
}

export function getAcidicModelReference(expr: Expression): AcidicModel | undefined {
  if (isReferenceExpr(expr) && isAcidicModel(expr.target.ref)) {
    return expr.target.ref;
  }
  if (isMemberAccessExpr(expr) && isAcidicModel(expr.member.ref)) {
    return expr.member.ref;
  }
  return undefined;
}

export function getAcidicObjectFieldReference(expr: Expression): AcidicObjectField | undefined {
  if (isReferenceExpr(expr) && isAcidicObjectField(expr.target.ref)) {
    return expr.target.ref;
  }
  if (isMemberAccessExpr(expr) && isAcidicObjectField(expr.member.ref)) {
    return expr.member.ref;
  }
  return undefined;
}

export function getAcidicQueryReference(expr: Expression): AcidicQuery | undefined {
  if (isReferenceExpr(expr) && isAcidicQuery(expr.target.ref)) {
    return expr.target.ref;
  }
  if (isMemberAccessExpr(expr) && isAcidicQuery(expr.member.ref)) {
    return expr.member.ref;
  }
  return undefined;
}

export function getAcidicMutationReference(expr: Expression): AcidicMutation | undefined {
  if (isReferenceExpr(expr) && isAcidicMutation(expr.target.ref)) {
    return expr.target.ref;
  }
  if (isMemberAccessExpr(expr) && isAcidicMutation(expr.member.ref)) {
    return expr.member.ref;
  }
  return undefined;
}

export function getAcidicSubscriptionReference(expr: Expression): AcidicSubscription | undefined {
  if (isReferenceExpr(expr) && isAcidicSubscription(expr.target.ref)) {
    return expr.target.ref;
  }
  if (isMemberAccessExpr(expr) && isAcidicSubscription(expr.member.ref)) {
    return expr.member.ref;
  }
  return undefined;
}

export function getAcidicEventReference(expr: Expression): AcidicEvent | undefined {
  if (isReferenceExpr(expr) && isAcidicEvent(expr.target.ref)) {
    return expr.target.ref;
  }
  if (isMemberAccessExpr(expr) && isAcidicEvent(expr.member.ref)) {
    return expr.member.ref;
  }
  return undefined;
}

 ...
 */
export default class AcidicObjectValidator implements AstValidator<AcidicObject> {
  validate(acidicObject: AcidicObject, accept: ValidationAcceptor): void {
    this.validateBaseAbstractModel(acidicObject, accept);
    validateDuplicatedDeclarations(acidicObject.$resolvedFields, accept);
    this.validateAttributes(acidicObject, accept);
    this.validateFields(acidicObject, accept);
  }

  private validateFields(acidicObject: AcidicObject, accept: ValidationAcceptor) {
    const idFields = acidicObject.$resolvedFields.filter((f) =>
      f.attributes.find((attr) => attr.decl.ref?.name === "@id")
    );
    const uniqueFields = acidicObject.$resolvedFields.filter((f) =>
      f.attributes.find((attr) => attr.decl.ref?.name === "@unique")
    );
    const modelLevelIds = getModelIdFields(acidicObject);
    const modelUniqueFields = getAcidicObjectUniqueFields(acidicObject);
    ...
    if (
      ...
      const { allows, denies, hasFieldValidation } = analyzePolicies(acidicObject);
    ...
    acidicObject.fields.forEach((field) => this.validateField(field, accept));

    if (!acidicObject.isAbstract) {
      acidicObject.$resolvedFields
        .filter((x) => isAcidicObject(x.type.reference?.ref))
        .forEach((y) => {
  ...
  private validateAttributes(dm: AcidicObject, accept: ValidationAcceptor) {
  ...
  private isSelfRelation(field: AcidicObjectField, relationName?: string) {
    ...
    if (relationName) {
      ...
      // points back
      const oppositeModel = field.type.reference?.ref as AcidicObject;
      if (oppositeModel) {
  ...
  private validateRelationField(field: AcidicObjectField, accept: ValidationAcceptor) {
    ...
    if (oppositeFields.length === 0) {
      ...
      let relationAcidicObjectName: string;

      if (field.$isInherited) {
        info.property = "name";
        const container = field.$container as AcidicObject;
        ...
        relationAcidicObjectName = abstractContainer.name;
      } else {
        relationFieldDocUri = getDocument(field).textDocument.uri;
        relationAcidicObjectName = field.$container.name;
      }

      const data: MissingOppositeRelationData = {
        relationFieldName: field.name,
        relationAcidicObjectName,
        relationFieldDocUri,
    ...
    if (relationOwner !== field && !relationOwner.type.array) {
      ...
      const containingModel = field.$container as AcidicObject;
  ...
  private validateBaseAbstractModel(model: AcidicObject, accept: ValidationAcceptor) {
...
export interface MissingOppositeRelationData {
  ...
  // the name of AcidicObject that the relation field belongs to.

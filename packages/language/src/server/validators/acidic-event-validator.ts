 ...
 */
export default class AcidicEventValidator implements AstValidator<AcidicEvent> {
  validate(acidicEvent: AcidicEvent, accept: ValidationAcceptor): void {
    this.validateBaseAbstractModel(acidicEvent, accept);
    validateDuplicatedDeclarations(acidicEvent.$resolvedFields, accept);
    this.validateAttributes(acidicEvent, accept);
    this.validateFields(acidicEvent, accept);
  }

  private validateFields(acidicEvent: AcidicEvent, accept: ValidationAcceptor) {
    const idFields = acidicEvent.$resolvedFields.filter((f) =>
      f.attributes.find((attr) => attr.decl.ref?.name === "@id")
    );
    const uniqueFields = acidicEvent.$resolvedFields.filter((f) =>
      f.attributes.find((attr) => attr.decl.ref?.name === "@unique")
    );
    const modelLevelIds = getModelIdFields(acidicEvent);
    const modelUniqueFields = getAcidicObjectUniqueFields(acidicEvent);
    ...
    acidicEvent.fields.forEach((field) => this.validateField(field, accept));

    if (!acidicEvent.isAbstract) {
      acidicEvent.$resolvedFields
        .filter((x) => isAcidicEvent(x.type.reference?.ref))
        .forEach((y) => {
  ...
  private validateAttributes(dm: AcidicEvent, accept: ValidationAcceptor) {
  ...
  private isSelfRelation(field: AcidicObjectField, relationName?: string) {
    ...
    if (relationName) {
      ...
      // points back
      const oppositeModel = field.type.reference?.ref as AcidicEvent;
      if (oppositeModel) {
  ...
  private validateRelationField(field: AcidicObjectField, accept: ValidationAcceptor) {
    ...
    if (relationOwner !== field && !relationOwner.type.array) {
      ...
      const containingModel = field.$container as AcidicEvent;
  ...
  private validateBaseAbstractModel(model: AcidicEvent, accept: ValidationAcceptor) {
...
export interface MissingOppositeRelationData {
  ...
  // the name of AcidicEvent that the relation field belongs to.

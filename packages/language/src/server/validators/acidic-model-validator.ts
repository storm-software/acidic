...
import { type AstNode, type DiagnosticInfo, getDocument, type ValidationAcceptor } from "langium";

/**
 * Validates data model declarations.
 */
export default class AcidicModelValidator implements AstValidator<AcidicModel> {
  validate(acidicModel: AcidicModel, accept: ValidationAcceptor): void {
    this.validateBaseAbstractModel(acidicModel, accept);
    validateDuplicatedDeclarations(acidicModel.$resolvedFields, accept);
    this.validateAttributes(acidicModel, accept);
    this.validateFields(acidicModel, accept);
  }

  private validateFields(acidicModel: AcidicModel, accept: ValidationAcceptor) {
    const idFields = acidicModel.$resolvedFields.filter((f) =>
      f.attributes.find((attr) => attr.decl.ref?.name === "@id")
    );
    const uniqueFields = acidicModel.$resolvedFields.filter((f) =>
      f.attributes.find((attr) => attr.decl.ref?.name === "@unique")
    );
    const modelLevelIds = getModelIdFields(acidicModel);
    const modelUniqueFields = getAcidicObjectUniqueFields(acidicModel);
    ...
    if (
      ...
      const { allows, denies, hasFieldValidation } = analyzePolicies(
    ...
    acidicModel.fields.forEach((field) => this.validateField(field, accept));

    if (!acidicModel.isAbstract) {
      acidicModel.$resolvedFields
        .filter((x) => isAcidicModel(x.type.reference?.ref))
        .forEach((y) => {
  ...
  private validateField(field: AcidicObjectField, accept: ValidationAcceptor): void {
  ...
  private validateAttributes(dm: AcidicModel, accept: ValidationAcceptor) {
    dm.attributes.forEach((attr) => {
      validateAttributeApplication(attr, accept);
    });
  }

  private parseRelation(field: AcidicObjectField, accept?: ValidationAcceptor) {
    ...
    if (!fields || !references) {
      ...
      if (fields.length !== references.length) {
        ...
        for (let i = 0; i < fields.length; i++) {
          if (fields[i] && !fields[i]?.$resolvedType) {
            if (accept) {
              accept("error", "field reference is unresolved", {
                node: fields[i] as AstNode
              });
            }
          }

          if (references[i] && !references[i]?.$resolvedType) {
            if (accept) {
              accept("error", "field reference is unresolved", {
                node: references[i] as AstNode
              });
            }
          }
  ...
  private isSelfRelation(field: AcidicObjectField, relationName?: string) {
    ...
    if (relationName) {
      ...
      // points back
      const oppositeModel = field.type.reference?.ref as AcidicModel;
      if (oppositeModel) {
        const oppositeModelFields = oppositeModel.$resolvedFields as AcidicObjectField[];
  ...
  private validateRelationField(field: AcidicObjectField, accept: ValidationAcceptor) {
    ...
    if (oppositeFields.length === 0) {
      ...
      let relationAcidicModelName: string;

      if (field.$isInherited) {
        info.property = "name";
        const container = field.$container as AcidicModel;
        const abstractContainer = {
          ...container.superTypes.find((x) => x.ref?.fields.find((f) => f.name === field.name))
            ?.ref,
          $type: "AcidicModel",
          isView: false
        } as AcidicModel;
        ...
        relationAcidicModelName = abstractContainer.name;
      } else {
        relationFieldDocUri = getDocument(field).textDocument.uri;
        relationAcidicModelName = field.$container.name;
      }

      const data: MissingOppositeRelationData = {
        relationFieldName: field.name,
        relationAcidicModelName,
        relationFieldDocUri,
    ...
    let relationOwner: AcidicObjectField;
    if (oppositeField && thisRelation?.references?.length && thisRelation.fields?.length) {
      ...
      if (!(field.type.array && oppositeField?.type.array)) {
        [field, oppositeField].forEach((f) => {
          if (f && !this.isSelfRelation(f, thisRelation.name)) {
            accept(
              "error",
    ...
    if (relationOwner !== field && !relationOwner.type.array) {
      ...
      const containingModel = field.$container as AcidicModel;
      const uniqueFieldList = getUniqueFields(containingModel);

      thisRelation.fields?.forEach((ref) => {
        const refField = ref.target.ref as AcidicObjectField;
        if (refField) {
          if (
  ...
  private validateBaseAbstractModel(model: AcidicModel, accept: ValidationAcceptor) {
    model.superTypes.forEach((superType, index) => {
      if (!superType.ref?.isAbstract)
        accept(
          "error",
          `Model ${superType.$refText} cannot be extended because it's not abstract`,
          {
            node: model,
            property: "superTypes",
            index
...
export interface MissingOppositeRelationData {
  ...
  // the name of AcidicModel that the relation field belongs to.

import { AcidicGeneratedModule, AcidicGeneratedSharedModule } from "../language";
import {
...
import { AcidicValidationRegistry, AcidicValidator } from "./validators/acidic-validator";
 ...
 */
export const AcidicModule: Module<AcidicServices, PartialLangiumServices & AcidicAddedServices> = {
  references: {
...
export function createSharedModule(
  ...
  return {
    ...
    workspace: {
      ...
      TextDocuments: (_services: any) => new TextDocuments(TextDocument) as any,
...
export function createAcidicServices(context: DefaultSharedModuleContext): {
  ...
  const shared = inject(createSharedModule(context), AcidicGeneratedSharedModule);

  const Acidic = inject(createDefaultModule({ shared }), AcidicGeneratedModule, AcidicModule);

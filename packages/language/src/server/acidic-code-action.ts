...
import type { AcidicFormatter } from "./acidic-formatter";
import type { MissingOppositeRelationData } from "./validators/acidic-model-validator";
  ...
  private fixMissingOppositeRelation(
    ...
    const rootCst =
      data.relationFieldDocUri === document.textDocument.uri
        ...
        : this.documents.all.find((doc) => doc.textDocument.uri === data.relationFieldDocUri)
            ?.parseResult.value;
    ...
    if (rootCst) {
      ...
      )?.fields.find((x) => x.name === data.relationFieldName) as AcidicObjectField;
          ...
          edit: {
            changes: {
              [targetDocument.textDocument.uri]: [
                {
                  range: {
                    start: lastField?.$cstNode?.range.end as Position,
                    end: lastField?.$cstNode?.range.end as Position
                  },
                  newText
                }
              ]

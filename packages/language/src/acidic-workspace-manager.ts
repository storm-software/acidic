...
import { isPlugin, Model } from "./__generated__/ast";
  ...
  override async initializeWorkspace(
    ...
    await Promise.all(
      folders
        ...
        .map(async entry =>
          this.traverseFolder(...entry, fileExtensions, collector)
        )
    );
    ...
    if (this.pluginModels.size > 0) {
      ...
      await Promise.all(
        folders
          .map(wf => [wf, this.getRootFolder(wf)] as [WorkspaceFolder, URI])
          .map(async entry =>
            this.loadPluginModels(...entry, unLoadedPluginModels, collector)
          )
      );

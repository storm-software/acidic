...
A utility package containing helpers for working with the Acidic definition
...
```bash
npm install -D @acidic/definition
```
...
Run `nx build definition` to build the library.

### Running unit tests

Run `nx test definition` to execute the unit tests via [Jest](https://jestjs.io).

### Linting

Run `nx lint definition` to run [ESLint](https://eslint.org/) on the package.

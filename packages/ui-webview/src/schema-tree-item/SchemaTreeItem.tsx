import { Commands } from "@acidic/static";
  ...
  useEffect(() => {
    const listener = (event: MessageEvent) => {
      if (event.data) {
        ...
        if (
          command === Commands.INIT_SCHEMA_START &&
          data?.service === service
        ) {
          setIsRefreshing(true);
        }
        if (command === Commands.INIT_SCHEMA_END && data?.service === service) {
          setIsRefreshing(false);
        } else if (
          command === Commands.REFRESH_SCHEMA_START &&
          data?.service === service
        ) {
          setIsRefreshing(true);
        } else if (
          command === Commands.REFRESH_SCHEMA_END &&
          data?.service === service
        ) {
          setIsRefreshing(false);
        }

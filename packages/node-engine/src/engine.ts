...
} from "@acidic/language/utilities";
import {
    ...
    try {
      ...
      const results = await Promise.all(
        context.definition.service.plugins.map(plugin => {
          try {
            if (plugin.provider) {
              context.plugin = this._pluginManager.getInstance(
                plugin.provider,
                plugin.options
              );
      ...
      const resultsLineWidth = context.definition.service.plugins.reduce(
        (ret: number, plugin) => {

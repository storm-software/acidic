...
export class AcidicDefinitionWrapper {
  ...
  private _mapAcidicSchemaToDefinition = (
    ...
    if (acidicObjects.length > 0) {
      ...
      for (const acidicObject of acidicObjects) {
        ...
        if (objectDefinition) {
          objectDefinition.attributes = acidicObject.attributes.map(
            this._mapAttribute
          );
        }
      }
    ...
    if (acidicModels.length > 0) {
      ...
      for (const acidicModel of acidicModels) {
        const modelDefinition = this.#objects.find(
          existing =>
            existing.name?.toUpperCase() === acidicModel.name?.toUpperCase()
        );
        if (modelDefinition) {
          modelDefinition.attributes = acidicModel.attributes.map(
            this._mapAttribute
          );
        }
      }
  ...
  private _mapObjectField = (
    ...
    if (isAcidicEnum(acidicField.type.reference?.ref)) {
      ...
      if (acidicField.type.reference?.ref) {
        enumDefinitionField.ref = this._mapEnum(
      ...
      const stringField: StringObjectFieldDefinition =
        schemaField as StringObjectFieldDefinition;
      ...
      const numberField: NumberObjectFieldDefinition =
        schemaField as NumberObjectFieldDefinition;
  ...
  private _mapModel = (acidicModel: AcidicModel): ModelDefinition => {
    const objectInput: ObjectDefinition = this._mapObject(

...
# Acidic Engine Package

This package contains the node-engine functionality of Acidic. It is used by various other Acidic packages to run their processes.
...
Run `nx build node-engine` to build the library.

### Running unit tests

Run `nx test node-engine` to execute the unit tests via [Jest](https://jestjs.io).

### Linting

Run `nx lint node-engine` to run [ESLint](https://eslint.org/) on the package.

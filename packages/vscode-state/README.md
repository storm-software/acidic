...
```bash
pnpm add -D @acidic/vscode-state
```
...
```bash
npm install -D @acidic/vscode-state
```
...
```bash
yarn add -D @acidic/vscode-state
```
...
Run `nx build vscode-state` to build the library.

### Running unit tests

Run `nx test vscode-state` to execute the unit tests via [Jest](https://jestjs.io).

### Linting

Run `nx lint vscode-state` to run [ESLint](https://eslint.org/) on the package.

import { AcidicSchema, isAcidicDataSource } from "@acidic/language/ast";
import {
  PLUGIN_MODULE_NAME,
  STD_LIB_MODULE_NAME
} from "@acidic/language/constants";
import { AstValidator } from "@acidic/language/types";
import { validateDuplicatedDeclarations } from "@acidic/language/utils";
 ...
 */
export default class AcidicSchemaValidator
  implements AstValidator<AcidicSchema>
{
  ...
  private validateDataSources(
    ...
    const dataSources = getAllDeclarationsFromImports(
      this.documents,
      schema
    ).filter(d => isAcidicDataSource(d));

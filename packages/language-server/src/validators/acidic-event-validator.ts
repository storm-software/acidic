import {
  ...
  isLiteralExpr,
  ReferenceExpr
} from "@acidic/language/__generated__/ast";
import { IssueCodes, SCALAR_TYPES } from "@acidic/language/constants";
...
export default class AcidicEventValidator implements AstValidator<AcidicEvent> {
  ...
  private isSelfRelation(field: AcidicObjectField, relationName?: string) {
    ...
    if (relationName) {
      ...
      if (oppositeModel) {
        const oppositeModelFields =
          oppositeModel.$resolvedFields as AcidicObjectField[];
  ...
  private validateRelationField(
    ...
    if (oppositeFields.length === 0) {
      ...
      if (field.$isInherited) {
        ...
        const abstractContainer = container.superTypes.find(
          x => x.ref?.fields.find(f => f.name === field.name)

...
A package containing the Acidic language server functionality
...
```bash
pnpm add -D @acidic/language-server
```
...
```bash
npm install -D @acidic/language-server
```
...
```bash
yarn add -D @acidic/language-server
```
...
Run `nx lint language-server` to run [ESLint](https://eslint.org/) on the package.

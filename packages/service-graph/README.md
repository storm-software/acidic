...
Library containing code for displaying schema graph in Visual Studio Code extension
...
```bash
pnpm add -D @acidic/service-graph
```
...
```bash
npm install -D @acidic/service-graph
```
...
```bash
yarn add -D @acidic/service-graph
```
...
Run `nx build service-graph` to build the library.

### Running unit tests

Run `nx test service-graph` to execute the unit tests via [Jest](https://jestjs.io).

### Linting

Run `nx lint service-graph` to run [ESLint](https://eslint.org/) on the package.

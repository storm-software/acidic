...
export async function getos(): Promise<GetOSResult> {
  ...
  const libsslSpecificPaths = computeLibSSLSpecificPaths({
    arch,
    archFromUname,
...
export async function resolveDistro(): Promise<DistroInfo> {
  ...
  try {
    const osReleaseInput = await fs.readFile(osReleaseFile, {
      encoding: "utf-8"
    });
 ...
 */
export function parseOpenSSLVersion(
  input: string
): GetOsResultLinux["libssl"] | undefined {
...
export function computeLibSSLSpecificPaths(
  ...
  return match(args)
    ...
    .with({ familyDistro: "debian" }, ({ archFromUname }) => {
      ...
      return [
        `/usr/lib/${archFromUname}-linux-gnu`,
        `/lib/${archFromUname}-linux-gnu`
      ];
    })
    ...
    .otherwise(({ familyDistro, arch, archFromUname }) => {
      ...
      debug(
        `Don't know any platform-specific paths for "${familyDistro}" on ${arch} (${archFromUname})`
      );
      return [];
    });
}
...
export async function getSSLVersion(
  ...
  const libsslFilenameFromSpecificPath: string | undefined =
    await findLibSSLInLocations(libsslSpecificPaths);

  if (libsslFilenameFromSpecificPath) {
    debug(
      `Found libssl.so file using platform-specific paths: ${libsslFilenameFromSpecificPath}`
    );
  ...
  if (!libsslFilename) {
     ...
     */
    libsslFilename = await findLibSSLInLocations([
      "/lib64",
      "/usr/lib64",
      "/lib"
    ]);
  ...
  if (libsslFilename) {
    debug(
      `Found libssl.so file using "ldconfig" or other generic paths: ${libsslFilename}`
    );
  ...
  const openSSLVersionLine: string | undefined =
    await getCommandOutput("openssl version -v");
...
async function findLibSSL(directory: string) {
  try {
    ...
    return dirContents.find(
      value =>
        value.startsWith("libssl.so.") && !value.startsWith("libssl.so.0")
    );
  } catch (e) {
    if (e.code === "ENOENT") {
      return undefined;
...
function isPlatformInfoDefined(
  args: Partial<PlatformInfo>
): args is PlatformInfo {
...
export async function getPlatformInfoMemoized(): Promise<
  PlatformInfo & { memoized: boolean }
> {
 ...
 */
export function getBinaryTargetForCurrentPlatformInternal(
  args: GetOSResult
): BinaryTarget {
  const {
    platform,
...
${additionalMessage}`
  ...
  if (platform === "linux" && targetDistro === undefined) {
    debug(
      `Distro is "${originalDistro}". Falling back to Prisma engines built for "${defaultDistro}".`
    );
  }
  ...
  if (platform === "linux" && arch === "arm64") {
    ...
    const baseName =
      targetDistro === "musl" ? "linux-musl-arm64" : "linux-arm64";
...
function isLibssl1x(
  libssl: NonNullable<GetOSResult["libssl"]> | string
): libssl is "1.0.x" | "1.1.x" {

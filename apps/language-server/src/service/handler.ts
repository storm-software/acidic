...
export function handleDiagnosticsRequest(
  ...
  for (const diag of res) {
    const previewNotKnownRegex =
      /The preview feature \"[a-zA-Z]+\" is not known/;
...
export function handleCompletionRequest(
  ...
  return (
    prismaSchemaWasmCompletions(params, document, onError) ||
    localCompletions(params, document, onError)
  );
}
...
export function handleRenameRequest(
  ...
  const isDatamodelBlockRename = isDatamodelBlockName(
    position,
    block,
    lines,
    document
  ...
  if (isDatamodelBlockRename || isEnumValueRename || isValidFieldRename) {
    ...
    if (isDatamodelBlockRename) {
      ...
      const matchBlockBeginning = new RegExp(
        `\\s*(${block.type})\\s+(${currentName})\\s*({)`,
        "g"
      );
      lineNumberOfDefinition = lines.findIndex(l =>
        matchBlockBeginning.test(l)
      );

...
- Describe your whole service in a single model, but allow for fined grained
  control (database structure, Api requests/response, validations, auth, etc.)
...
**Acidic** refers to a collection of applications and libraries that are used to
build server-side code from a user-defined model. The specification for this
language can be found in the monorepo's [language package](/packages/language/).
...
**Acidic** has it's own Visual Studio Code extension to support the language
model in the IDE. The extension can be found in the
